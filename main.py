...
config = Config.load()
chat = {"channels": {"0": {"name": "General", "messages": []}}}
messages = []
mqtt_messages = []
...
def prune_expired_nodes():
    ...
    for id, node in nodes.items():
        last_seen = (
            datetime.datetime.fromisoformat(node["last_seen"]).astimezone()
        ...
        if (
            node["active"]
            and since >= config["server"]["node_activity_prune_threshold"]
        ):
...
def handle_text(client, userdata, msg):
    ...
    id = "!" + f'{msg["from"]:x}'
    to = "!" + f'{msg["to"]:x}'
    chat["channels"][str(msg["channel"])]["messages"].insert(
        0,
        {
            "id": msg["id"],
            "sender": msg["sender"],
...
def _serialize_node(node):
    ...
    last_seen = (
        node["last_seen"]
        if isinstance(node["last_seen"], datetime.datetime)
        else datetime.datetime.fromisoformat(node["last_seen"])
    )
    serialized = {
        "id": node["id"],
        ...
        "last_seen": last_seen,
        "since": datetime.datetime.now(ZoneInfo(config["server"]["timezone"]))
        - last_seen,
    }
    server_node = nodes[f'{config["server"]["node_id"]}']
    if (
        server_node
        and "position" in server_node
        and server_node["position"]
        and server_node["position"]["latitude_i"] != 0
        and server_node["position"]["longitude_i"] != 0
        and node["position"]
        and node["position"]["latitude_i"] != 0
        and node["position"]["longitude_i"] != 0
    ):
        serialized["distance_from_host_node"] = round(
...
def _serialize_neighborinfo(node):
    ...
    ni["neighbors"] = (
        _serialize_neighborinfo_neighbors(node) if "neighbors" in ni else None
    )
    return ni
...
def render_static_html_files():
    ...
    rendered_html = template.render(
        ...
        zoneinfo=ZoneInfo(config["server"]["timezone"]),
        timestamp=datetime.datetime.now(ZoneInfo(config["server"]["timezone"])),
    )
    ...
    template = env.get_template(
        f'{config["paths"]["templates"]}/static/mesh_log.html.j2'
    )
    rendered_html = template.render(
        config=config,
        messages=messages,
    ...
    env = Environment(loader=FileSystemLoader("."), autoescape=True)
    template = env.get_template(
        f'{config["paths"]["templates"]}/static/mqtt_log.html.j2'
    )
    rendered_html = template.render(
        config=config,
    ...
    for id, node in nodes.items():
        if (
            "active" in node
            and node["active"]
            and "neighborinfo" in node
            and node["neighborinfo"]
        ):
            active_nodes_with_neighbors[id] = _serialize_node(node)
    env = Environment(loader=FileSystemLoader("."), autoescape=True)
    template = env.get_template(
        f'{config["paths"]["templates"]}/static/neighbors.html.j2'
    )
    rendered_html = template.render(
        config=config,
        nodes=nodes,
    ...
    env = Environment(loader=FileSystemLoader("."), autoescape=True)
    template = env.get_template(
        f'{config["paths"]["templates"]}/static/telemetry.html.j2'
    )
    rendered_html = template.render(
        config=config,
        nodes=nodes,
    ...
    env = Environment(loader=FileSystemLoader("."), autoescape=True)
    template = env.get_template(
        f'{config["paths"]["templates"]}/static/traceroutes.html.j2'
    )
    rendered_html = template.render(
        config=config,
        nodes=nodes,
...
def save():
    ...
    last_data = (
        config["server"]["last_data_save"]
        if "last_data_save" in config["server"]
        else config["server"]["start_time"]
    )
    since_last_data = (save_start - last_data).total_seconds()
    last_render = (
        config["server"]["last_render"]
        if "last_render" in config["server"]
        else config["server"]["start_time"]
    )
...
def save_node_infos_to_file(node_infos, type, path, config=config):
    ...
    if type == "html":
        env = Environment(loader=FileSystemLoader("."), autoescape=True)
        template = env.get_template(
            f'{config["paths"]["templates"]}/static/node_infos.html.j2'
        )
        rendered_html = template.render(
...
def load():
    ...
    try:
        ...
        print(
            f"Loaded {len(nodes)} existing nodes from file ({config['paths']['data']}/nodes.json)"
        )
    except FileNotFoundError:
        nodes = {}
    if config["server"]["node_id"] not in nodes:
        nodes[config["server"]["node_id"]] = Node.default_node(
    ...
    try:
        ...
        print(
            f"Loaded {len(chat['channels']['0']['messages'])} chat messages from file ({config['paths']['data']}/chat.json)"
        )
    except FileNotFoundError:
        chat = {"channels": {"0": {"name": "General", "messages": []}}}
    ...
    try:
        ...
        for msg in telemetry:
            ...
            telemetry_by_node[id].insert(0, msg)
        print(
            f"Loaded {len(telemetry)} telemetry messages from file ({config['paths']['data']}/telemetry.json)"
        )
    ...
    try:
        ...
        for msg in traceroutes:
            ...
            traceroutes_by_node[id].insert(0, msg)
        print(
            f"Loaded {len(traceroutes)} traceroutes from file ({config['paths']['data']}/traceroutes.json)"
        )
...
def save_chat_to_file(chat, type, path, config=config):
    ...
    if type == "html":
        env = Environment(loader=FileSystemLoader("."), autoescape=True)
        template = env.get_template(
            f'{config["paths"]["templates"]}/static/chat.html.j2'
        )
        rendered_html = template.render(
            config=config,
            nodes=nodes,
...
async def start_mqtt_processor():
    global chat
    global config
    global nodes
...
async def main():
    async with asyncio.TaskGroup() as tg:
        ...
        tg.create_task(api.start_uvicorn(loop))
        tg.create_task(start_mqtt_processor())
